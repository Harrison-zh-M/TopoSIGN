...
def flipping(graph):
    _from_sparse = getattr(nx, 'from_scipy_sparse_array', None) or getattr(nx, 'from_scipy_sparse_matrix')
    _to_sparse   = getattr(nx, 'to_scipy_sparse_array',   None) or getattr(nx, 'to_scipy_sparse_matrix')
    graph_1 = _from_sparse(graph, create_using=nx.DiGraph)
    graph_bidirectional = _from_sparse(coo_matrix((graph_1.number_of_nodes(), graph_1.number_of_nodes()),
                                               dtype=np.int8), create_using=nx.DiGraph)
    dictionary = dictionary_connection(graph_1)
    biconnections = biconnection(graph_1)
    A = _to_sparse(graph_1)
    n_rows = A.shape[0]
    lista = [add_edges(graph_bidirectional, n_rows, biconnections, dictionary, i) for i in range(0, len(biconnections), 2)]
    return _to_sparse(graph_bidirectional)
