...
class CONVOL(nn.Module):
    ...
    def __init__(self, hop: int, directed: bool = False):
        ...
        if self._undirected:
            self._w_p = Parameter(torch.FloatTensor(self._hop_p+2, 1))
            self._w_n = Parameter(torch.FloatTensor(self._hop_n+3, 1))
            self._w_p_all = Parameter(torch.FloatTensor(32, 32))
            self._w_n_all = Parameter(torch.FloatTensor(32, 32))
    ...
    def forward(self, A_p: Union[torch.FloatTensor, torch.sparse_coo_tensor],
        ...
        if self._undirected:
            ...
            feat_p = self._w_p[2] * torch.matmul(A_p, torch.matmul(A_p, curr_p)) + self._w_p[1] * torch.matmul(A_p, curr_p) + self._w_p[0] * x_p
            feat_n = self._w_n[2] * torch.matmul(A_p, torch.matmul(A_n, curr_n_aux)) + self._w_n[1] * torch.matmul(A_n, torch.matmul(A_p, curr_n_aux)) + self._w_n[0] * torch.matmul(A_n, curr_n_aux)
            # feat_p = self._w_p[2] * torch.matmul(A_p, torch.matmul(A_p, curr_p)) + self._w_p[1] * torch.matmul(A_p, curr_p) + self._w_p[0] * x_p + self._w_p[3] * torch.matmul(A_n, torch.matmul(A_n, curr_p))


            feat = torch.cat([feat_p, feat_n], dim=1)
            return feat, feat_p, feat_n
        else:
            A_sp = A_p
            A_sn = A_n
            A_tp = A_pt
...
class DSGC(nn.Module):
    ...
    def forward(self, A_p: Union[torch.FloatTensor, torch.sparse_coo_tensor],
        ...
        if self._undirected:
            ...
            z, z_p, z_n = self._CONVOL(A_p, A_n, x_p, x_n)
        else:
            # MLP
            ...
            z = self._CONVOL(A_p, A_n, x_sp, x_sn, x_tp, x_tn, A_pt, A_nt)
            z_p = None
            z_n = None
        ...
        prob_mlp = F.softmax(output, dim=1)


        return z, z_p, z_n, F.normalize(z), predictions_cluster, prob_mlp
...
class Balance_Theory(nn.Module):
    r"""The signed graph clustering model with balance theory, restricted to 2 hops for fair comparison.
